...
def aantal_kluizen_vrij():
    ...
    """
    aantal = open(__my_test_file())
    return 12 - len(aantal.readlines())


def nieuwe_kluis():
    """
    Indien er nog kluizen vrij zijn --> Kluiscode invoeren.
    Deze kluiscode moet
    -   uit minimaal 4 tekens bestaan,
    -   puntkomma (';') mag er niet in
    ...
    een tekstbestand.
    De returnwaarde van de functie is dan gelijk aan het toegekende
    kluisnummer.
    Returns:
...
def development_code():
    ...
    while not stat:
        print("\n1: Aantal kluizen vrij")
        print("2: Nieuwe kluis aanvragen")
        print("3: Kluis uithalen")
        print("4: Kluis inleveren")
        print("\n5: Programma Afsluiten en Testen")
        choice = input("\nGeef keuze op: >>> ")
        try:
            if int(choice) == 1:
                print('\nKluizen vrij:', aantal_kluizen_vrij())
            elif int(choice) == 2:
                print(nieuwe_kluis())
            elif int(choice) == 3:
                print(kluis_openen())
