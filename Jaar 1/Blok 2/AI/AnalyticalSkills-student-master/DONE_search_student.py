...
def binary_search(lst, target):
    ...
    return False"""
    indexstart = 0  # Copy-paste van eerdere uitwerking FA 2 - Algoritmiek -- D. Tjon-A-Njoek
    indexend = len(lst)
    while True:
        guess = int(indexend - indexstart // 1)
        if guess == indexend and indexstart == 0:
            guess = guess / 2  # pakt midden van guessrange voor index 0, anders guess == indexend

        if lst[int(guess)] < target:
            indexstart += 1
        else:
            indexend -= 1
...
def binary_search_index(lst, target):
    ...
    Returns:
        int: De index waar het element in de lijst staat, of -1 als het element niet in de lijst voorkomt.
    """
    indexstart = 0  # Copy-paste van eerdere uitwerking in FA 2 - Algoritmiek -- (c) D. Tjon-A-Njoek 2021
    indexend = len(lst)
    while True:
        guess = indexend - indexstart
        if guess == indexend and indexstart == 0:
            guess = guess / 2 // 1  # pakt midden van guessrange voor index 0, anders guess == indexend
...
def linear_search_index_steps(lst, target):
    ...
    for instance in lst:
        if instance == target:
            return lst.index(instance), steps
        steps += 1
    return -1, steps
